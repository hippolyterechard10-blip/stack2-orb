...
def log_trade(sleeve, symbol, direction, entry_price, exit_price, exit_reason,
              point_value, virtual_equity, backtest_slippage_ticks=config.BACKTEST_SLIPPAGE_TICKS,
              observed_slippage_ticks=None, tick=0.25):
    pnl_points = (exit_price - entry_price) * (1 if direction > 0 else -1)
    ...
    rec = {
        ...
        "pnl_points": round(pnl_points, 2), "pnl_dollars": round(pnl_dollars, 2),
    ...
    _std.info("TRADE %s %s %s entry=%s exit=%s reason=%s pnl=$%.2f eq=$%.2f%s",
              sleeve, rec["direction"], symbol, entry_price, exit_price, exit_reason,
              pnl_dollars, virtual_equity, "  ⚠️SLIPPAGE" if slip_alert else "")
    return rec
